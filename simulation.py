...
class Hrp4Controller(dart.gui.osg.RealTimeWorldNode):
    def __init__(self, world, hrp4, scene1, scene2, scene3, traj1, traj2):
        ...
        for step in self.footstep_planner.plan:
            ...
            self.step_patches.append(rect_curr) 
    ...
    def customPreStep(self):
        ...
        if self.scene1:
            ...
            if (self.time == start_time_passo) and (phase == 'ss') and (idx == 5):
                self.push_active_timer = 150  
                self.force_duration = 3       
        ...
        if self.scene2:
            ...
            if (self.time == start_time_passo) and (phase == 'ss') and (idx == 5):
                ...
                if swing_foot == 'rfoot':
                    # swing right -> y negative
                    self.lateral_push_dir = -10.0
                else:
                    # swing left -> y positive
                    self.lateral_push_dir = 10.0
                # ------------------------------------
            ...
            if hasattr(self, 'force_duration_s2') and self.force_duration_s2 > 0:
                push_force = np.array([15.0, self.lateral_push_dir, 0.0]) 
        ...
        if is_pushed:
            ...
            if target_idx_corr < len(self.footstep_planner.plan):
                ...
                print(f"[MPC solved] t={self.time} | Foot: {piede_in_arrivo} | "
                      f"Nominal: {nom_y:.4f} m -> Current: {curr_y:.4f} m | "
                      f"Displacement: {spostamento_y:.4f} m")
