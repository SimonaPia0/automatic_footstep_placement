...
class FootTrajectoryGenerator:
  ...
  def generate_feet_trajectories_at_time(self, time):
    ...
    if phase == 'ds':
        next_step_idx = min(step_index + 1, len(self.plan) - 1)
        ...
        swing_pose = np.hstack((
            self.plan[next_step_idx]['ang'],
            self.plan[next_step_idx]['pos']
        ))
        zero_vel = np.zeros(6)
        zero_acc = np.zeros(6)
        return {
    ...
    if step_index + 1 < len(self.plan):
        target_pos = self.plan[step_index + 1]['pos']
    else:
        target_pos = self.plan[step_index]['pos']
